...
Workflows must reflect controlled project behavior.

A registered draft is official and must be preserved.

Files are never replaced once registered.

Every revision action must be audited.

Current Revision is a pointer, not a status.

User authority is defined in the Roles and Permission Matrix.

Workflow steps should not silently mutate controlled records.

AI must not approve, reject, cancel, issue, supersede, or perform official CO/RCO actions.
